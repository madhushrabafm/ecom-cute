 ...
 * The URL and Key are provided by the user for the Luna DecorPremium Collective project.
 */
const supabaseUrl =
  process.env.SUPABASE_URL || "https://qahreermniosjngxqsnm.supabase.co";
const supabaseAnonKey =
  process.env.SUPABASE_ANON_KEY ||
  "sb_publishable_Emv9qEIa9NapUI9gKW1uzA_WWUZZlSf";
    ...
    const noOp: any = new Proxy(
      {},
      {
        get: (target, prop) => {
          return () => {
            console.warn(
              `Supabase call to ".${String(prop)}" ignored: Client not fully configured.`,
            );
...
export const handleSupabaseError = (error: any) => {
  ...
  return (
    error?.message || "An unexpected error occurred with the data collective."
  );
};
