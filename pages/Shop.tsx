    ...
    <div className="border-b border-gray-100 py-6 last:border-0">
      ...
      >
        <div className="flex items-center gap-3">
          <span className={`text-[11px] font-bold uppercase tracking-[0.2em] transition-colors ${openSections.includes(id) ? 'text-black' : 'text-gray-400 group-hover:text-black'}`}>
            {title}
          </span>
          {isActive && <div className="w-1.5 h-1.5 bg-black rounded-full"></div>}
        </div>
        <i className={`fa-solid fa-chevron-down text-[8px] transition-transform duration-500 ${openSections.includes(id) ? 'rotate-180 text-black' : 'text-gray-300'}`}></i>
      </button>
      <div className={`mt-6 space-y-4 overflow-hidden transition-all duration-500 ease-in-out ${openSections.includes(id) ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
        {children}
      </div>
    </div>
  );
    ...
    <div className="max-w-[1600px] mx-auto px-4 sm:px-8 lg:px-16 py-12 animate-fade-in relative">
      <div className="flex flex-col mb-12 space-y-8">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10">
          ...
          <div className="flex items-center gap-6">
            <div className="relative group border-b border-gray-100 py-2">
              <span className="absolute -top-4 left-0 text-[7px] font-bold uppercase tracking-widest text-gray-400">Sort By</span>
              <select 
                value={activeSort}
                onChange={(e) => updateParams('sort', e.target.value)}
                className="appearance-none bg-transparent pr-8 text-[10px] font-bold uppercase tracking-widest outline-none cursor-pointer"
              >
        ...
        {(activeCategory !== 'All' || activeBrand !== 'All' || activePriceMax < 15000) && (
          <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-gray-100 animate-slide-up">
            <span className="text-[9px] font-bold uppercase tracking-widest text-vogue-500 mr-2">Refining:</span>
            {activeCategory !== 'All' && (
              <button onClick={() => updateParams('category', 'All')} className="flex items-center gap-2 px-3 py-1.5 bg-black text-white rounded-full text-[9px] font-bold uppercase tracking-widest hover:bg-zinc-800 transition-all">
                {activeCategory} <i className="fa-solid fa-xmark ml-1"></i>
              </button>
            )}
            {activeBrand !== 'All' && (
              <button onClick={() => updateParams('brand', 'All')} className="flex items-center gap-2 px-3 py-1.5 bg-black text-white rounded-full text-[9px] font-bold uppercase tracking-widest hover:bg-zinc-800 transition-all">
                {activeBrand} <i className="fa-solid fa-xmark ml-1"></i>
              </button>
            )}
            {activePriceMax < 15000 && (
              <button onClick={() => updateParams('maxPrice', '15000')} className="flex items-center gap-2 px-3 py-1.5 bg-black text-white rounded-full text-[9px] font-bold uppercase tracking-widest hover:bg-zinc-800 transition-all">
                Under ₹{activePriceMax.toLocaleString()} <i className="fa-solid fa-xmark ml-1"></i>
              </button>
            )}
            <button onClick={clearAllFilters} className="text-[9px] font-bold uppercase tracking-widest text-black underline underline-offset-4 ml-2">Clear All</button>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-16">
        <aside className="hidden lg:block w-72 flex-shrink-0">
          <div className="sticky top-28 space-y-2">
            <FilterSection title="Categories" id="category" isActive={activeCategory !== 'All'}>
              <div className="flex flex-col gap-1">
                {['All', ...CATEGORIES].map(cat => (
                  ...
                  >
                    <span>{cat}</span>
                    <span className="opacity-40 text-[8px]">{cat === 'All' ? products.length : products.filter(p => p.category === cat).length}</span>
                  </button>
                ))}
              </div>
            </FilterSection>
            ...
            <FilterSection title="Price Limit" id="price" isActive={activePriceMax < 15000}>
              <div className="px-2 pt-2">
                <div className="flex justify-between items-end mb-6">
                  <span className="text-[9px] font-bold text-gray-300 tracking-widest uppercase text-left">Max Spend</span>
                  <span className="text-xl font-serif font-bold italic text-right">₹{activePriceMax.toLocaleString()}</span>
                </div>
                <input 
                  type="range" 
        ...
        <div className="flex-grow">
          {isLoadingProducts ? (
            ...
            <div className="py-40 text-center animate-fade-in">
              ...
              <h3 className="text-2xl font-serif font-bold italic mb-4">No pieces match your search.</h3>
              <p className="text-sm text-gray-400 font-light mb-8">Try adjusting your filters.</p>
              <button onClick={clearAllFilters} className="bg-black text-white px-10 py-4 text-[10px] font-bold uppercase tracking-widest">Reset Catalog</button>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="max-w-7xl mx-auto px-6 sm:px-12 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex flex-wrap items-center gap-6">
              <div className="hidden md:block">
                <p className="text-[10px] font-bold uppercase tracking-widest text-vogue-500 mb-1">Comparison Tray</p>
                <p className="text-xs font-serif italic">{comparisonList.length} of 4 items selected</p>
              </div>
              <div className="flex gap-4">
                {comparedProducts.map(product => (
                  <div key={product.id} className="relative group w-12 h-16 sm:w-16 sm:h-20 bg-gray-50 rounded-sm overflow-hidden border border-gray-100 shadow-sm">
            ...
            <div className="flex items-center gap-6 w-full md:w-auto">
              <button 
                ...
                className="text-[9px] font-bold uppercase tracking-widest text-vogue-500 hover:text-black transition-colors"
              >
                Reset Tray
              </button>
              <button 
