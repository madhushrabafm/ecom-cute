...
const StyleAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "ai",
      text: "Bonjour. I am your Luna DecorDigital Concierge. To provide truly personalized guidance, shall we review your current aesthetic preferences?",
    },
  ]);
  ...
  const handleSend = async (text: string = input) => {
    ...
    const context = {
      ...
      weather: userLocation
        ...
        : "Climate Controlled",
      currentProduct: sharedProduct
        ? `${sharedProduct.name} in ${sharedProduct.category}`
        : undefined,
    };

    const advice = await getFashionAdvice(
      text || "How should I style this?",
      context,
    );
    ...
    setMessages((prev) => [
      ...
      {
        role: "ai",
        text: advice || "An error occurred in my styling algorithms.",
      },
    ]);
    setLoading(false);
  };
      ...
      <div className="relative w-full max-w-md h-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-500">
        ...
        {showPreferences && (
          <div className="p-8 bg-zinc-900 text-white animate-in slide-in-from-top duration-300 border-b border-white/5">
            <h3 className="text-[10px] font-bold uppercase tracking-[0.3em] text-vogue-500 mb-6">
              Your Aesthetic Profile
            </h3>
            <div className="space-y-6">
              <div>
                <label className="block text-[8px] uppercase tracking-widest text-white/40 mb-3">
                  Core Aesthetic
                </label>
                <div className="flex flex-wrap gap-2">
                  {["Minimalist", "Avant-Garde", "Streetwear", "Old Money"].map(
                    (style) => (
                      <button
                        key={style}
                        onClick={() =>
              ...
              <div className="flex justify-between items-center pt-2">
                ...
                <div className="flex gap-4">
                  <div className="flex items-center gap-2 text-[9px] uppercase font-bold tracking-widest">
                    <i
                      className={`fa-solid fa-location-dot ${userLocation ? "text-green-500" : "text-red-500"}`}
                    ></i>
        ...
        >
          {messages.map((msg, i) => (
            <div
              key={i}
              className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"} animate-in slide-up duration-500`}
            >
              <div className="max-w-[85%] space-y-3">
                {msg.attachedProduct && (
                  <div className="bg-white border border-gray-100 p-3 shadow-md rounded-sm flex gap-4 mb-2 animate-in zoom-in-95">
                    <img
                      src={msg.attachedProduct.images[0]}
                    ...
                    <div className="flex flex-col justify-center">
                      ...
                      <span className="text-[9px] text-vogue-500 uppercase tracking-widest">
                        {msg.attachedProduct.subcategory}
                      </span>
                      <span className="text-[10px] font-bold mt-1">
                        ₹{msg.attachedProduct.price.toLocaleString()}
                      </span>
                    </div>
                  </div>
                )}
                <div
                  className={`p-6 rounded-sm text-sm leading-relaxed shadow-sm ${
                    msg.role === "user"
                ...
                {msg.role === "ai" && (
                  <div className="flex gap-3 px-2">
                    <button className="text-[8px] font-bold uppercase tracking-widest text-vogue-500 hover:text-black transition-colors">
                      Find Similar
                    </button>
                    <button className="text-[8px] font-bold uppercase tracking-widest text-vogue-500 hover:text-black transition-colors">
                      See In Shop
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-white p-6 rounded-sm shadow-sm flex gap-1 items-center border border-gray-100">
                ...
                <span className="text-[9px] font-bold uppercase tracking-widest text-vogue-500 ml-2">
                  Curating Advice...
                </span>
              </div>
            </div>
          ...
          <div className="px-8 py-4 bg-zinc-900 text-white flex justify-between items-center animate-slide-up border-t border-white/5">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 border border-white/10 p-1">
                <img
                  src={sharedProduct.images[0]}
                  className="w-full h-full object-cover grayscale"
                />
              </div>
              <div>
                <span className="text-[9px] uppercase font-bold tracking-[0.2em] text-vogue-500 block">
                  Consulting Context
                </span>
        ...
        <div className="px-8 py-5 flex gap-3 overflow-x-auto no-scrollbar border-t border-gray-100 bg-white">
          ...
          {["Night Out", "Heritage Mix", "Trend Report"].map((tag) => (
            <button
              key={tag}
              onClick={() =>
                handleSend(`How should I dress for a ${tag.toLowerCase()}?`)
              }
